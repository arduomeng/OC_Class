...
+ (void)requestExactImageSync:(PHAsset *)asset size:(CGSize)size resultHandler:(resultHandlerExactImage)block{
    ...
    @try {
        ...
        [ZhugeioManager zhugeEvent:[NSString stringWithFormat:@"assetID %@,  size %@, exception %@", asset.localIdentifier, NSStringFromCGSize(size), exception]];
        block(nil, nil);
    } @finally {
